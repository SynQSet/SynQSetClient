...
.EXAMPLE
    ...
    Will test the module in .\BuildOutput\SynQSetClient\0.1.0\
.EXAMPLE
    .\Test-Module.ps1
    Will test the latest  module version in .\BuildOutput\SynQSetClient\
#>
param (
    ...
    [ValidateScript({
            ...
            $b_Path = [IO.Path]::Combine($PSScriptRoot, 'BuildOutput', 'SynQSetClient')
...
begin {
    $TestResults = $null
    $BuildOutput = [IO.DirectoryInfo]::New([IO.Path]::Combine($PSScriptRoot, 'BuildOutput', 'SynQSetClient'))
    ...
    $BuildOutDir = Resolve-Path $([IO.Path]::Combine($PSScriptRoot, 'BuildOutput', 'SynQSetClient', $version)) -ErrorAction Ignore | Get-Item -ErrorAction Ignore
    if (!$BuildOutDir.Exists) { throw [System.IO.DirectoryNotFoundException]::new($BuildOutDir) }
    $manifestFile = [IO.FileInfo]::New([IO.Path]::Combine($BuildOutDir.FullName, "SynQSetClient.psd1"))
    ...
    if (!(Test-Path -Path $([IO.Path]::Combine($PSScriptRoot, "SynQSetClient.psd1")) -PathType Leaf -ErrorAction Ignore)) { throw [System.IO.FileNotFoundException]::New("Module manifest file Was not Found in '$($BuildOutDir.FullName)'.") }
    $script:fnNames = [System.Collections.Generic.List[string]]::New(); $testFiles = [System.Collections.Generic.List[IO.FileInfo]]::New()
    [void]$testFiles.Add([IO.FileInfo]::New([IO.Path]::Combine("$PSScriptRoot", 'Tests', 'SynQSetClient.Intergration.Tests.ps1')))
    [void]$testFiles.Add([IO.FileInfo]::New([IO.Path]::Combine("$PSScriptRoot", 'Tests', 'SynQSetClient.Features.Tests.ps1')))
    [void]$testFiles.Add([IO.FileInfo]::New([IO.Path]::Combine("$PSScriptRoot", 'Tests', 'SynQSetClient.Module.Tests.ps1')))
}

process {
    Get-Module SynQSetClient | Remove-Module
